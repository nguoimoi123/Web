...
export async function POST(req, { params }) {
    ...
    const { userId } = await params; // Lấy userId và shippingId từ URL params
    ...
    const { state, payment, id_shipping } = await req.json(); // Lấy dữ liệu từ body (không cần cart_code nữa)

    if (!state || !payment || !id_shipping) {
        return new Response(
    ...
    try {
        ...
        const [result] = await connection.execute(`
            ...
            VALUES (?, ?, ?, ?, ?)
        `, [cart_code, state, payment, userId, id_shipping]);
